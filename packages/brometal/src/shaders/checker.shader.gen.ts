...
const checkerShader: CompiledShader<{ aPosition: 'vec3'; aUv: 'vec2' }, Record<string, never>, { uTime: 'float'; uAspect: 'float' }> = {
...
void main() {
  vec2 q = rotate2(vec2((vUv.x - 0.5) * uAspect, vUv.y - 0.5), uTime * 0.2) * 6.0;
  float c = mod(floor(q.x) + floor(q.y), 2.0);
  float tone = mix(0.12, 0.92, c);
  fragColor = vec4(tone, tone * 0.9, tone, 1.0);
}
`,
