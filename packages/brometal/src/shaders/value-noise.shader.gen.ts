...
const valueNoiseShader: CompiledShader<{ aPosition: 'vec3'; aUv: 'vec2' }, Record<string, never>, { uTime: 'float'; uAspect: 'float' }> = {
...
void main() {
  vec2 p = vec2(vUv.x * uAspect, vUv.y) * 8.0;
  vec2 drift = vec2(uTime * 0.4, uTime * 0.15);
  float n = vnoise2(p + drift);
...
fn fs_main(bm_in : BmVSOut) -> @location(0) vec4f {
  let p = vec2f(bm_in.vUv.x * bm_u.uAspect, bm_in.vUv.y) * 8.0;
  let drift = vec2f(bm_u.uTime * 0.4, bm_u.uTime * 0.15);
  let n = vnoise2(p + drift);
  return vec4f(vec3f(n, n, n), 1.0);
}
`,
...
export default valueNoiseShader;
