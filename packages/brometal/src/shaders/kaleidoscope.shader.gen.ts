...
const kaleidoscopeShader: CompiledShader<{ aPosition: 'vec3'; aUv: 'vec2' }, Record<string, never>, { uTime: 'float'; uAspect: 'float' }> = {
...
void main() {
  ...
  float seg = abs(fract(angle * 6.0 + uTime * 0.05) - 0.5) * 2.0;
  float radius = length(p);
  float n = fbm2(vec2(seg * 3.0, radius * 5.0 - uTime * 0.4), 4.0);
  vec3 color = cosinePalette(n + radius, vec3(0.5, 0.5, 0.5), vec3(0.5, 0.5, 0.5), vec3(1.0, 1.0, 0.7), vec3(0.1, 0.45, 0.8));
...
fn fs_main(bm_in : BmVSOut) -> @location(0) vec4f {
  ...
  let seg = abs(fract(angle * 6.0 + bm_u.uTime * 0.05) - 0.5) * 2.0;
  let radius = length(p);
  let n = fbm2(vec2f(seg * 3.0, radius * 5.0 - bm_u.uTime * 0.4), 4.0);
  let color = cosinePalette(n + radius, vec3f(0.5, 0.5, 0.5), vec3f(0.5, 0.5, 0.5), vec3f(1.0, 1.0, 0.7), vec3f(0.1, 0.45, 0.8));
...
export default kaleidoscopeShader;
