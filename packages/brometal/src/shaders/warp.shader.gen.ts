...
const warpShader: CompiledShader<{ aPosition: 'vec3'; aUv: 'vec2' }, Record<string, never>, { uTime: 'float'; uAspect: 'float' }> = {
...
void main() {
  ...
  float n = warp2(p, uTime);
  vec3 color = cosinePalette(n * 1.3 + 0.05, vec3(0.5, 0.5, 0.5), vec3(0.5, 0.5, 0.5), vec3(1.0, 1.0, 1.0), vec3(0.0, 0.15, 0.35));
...
fn warp2(p : vec2f, time : f32) -> f32 {
  let q = vec2f(fbm2(p, 4.0), fbm2(p + vec2f(5.2, 1.3), 4.0));
  let shifted = p + q * 2.0;
  let r = vec2f(fbm2(shifted + vec2f(1.7, 9.2 + time * 0.15), 4.0), fbm2(shifted + vec2f(8.3 - time * 0.126, 2.8), 4.0));
  return fbm2(p + r * 2.0, 4.0);
}
...
fn fs_main(bm_in : BmVSOut) -> @location(0) vec4f {
  let p = vec2f(bm_in.vUv.x * bm_u.uAspect, bm_in.vUv.y) * 2.4;
  let n = warp2(p, bm_u.uTime);
  let color = cosinePalette(n * 1.3 + 0.05, vec3f(0.5, 0.5, 0.5), vec3f(0.5, 0.5, 0.5), vec3f(1.0, 1.0, 1.0), vec3f(0.0, 0.15, 0.35));
