...
const electricShader: CompiledShader<{ aPosition: 'vec3'; aUv: 'vec2' }, Record<string, never>, { uTime: 'float'; uAspect: 'float' }> = {
...
void main() {
  ...
  float flicker = 0.7 + 0.3 * hash11(floor(uTime * 14.0));
  float offset = (fbm2(vec2(vUv.y * 2.5 - uTime * 2.5, floor(uTime * 7.0)), 4.0) - 0.5) * 0.5;
  float d = abs(x - offset);
  float core = 0.006 / (d + 0.006);
  float glow = 0.04 / (d + 0.04);
  float bolt = pow(core, 1.5) * flicker;
  float halo = pow(glow, 2.0) * 0.5 * flicker;
  fragColor = vec4(bolt * 0.8 + halo * 0.4, bolt * 0.9 + halo * 0.5, bolt + halo, 1.0);
}
`,
...
fn fs_main(bm_in : BmVSOut) -> @location(0) vec4f {
  let x = (bm_in.vUv.x - 0.5) * bm_u.uAspect;
  let flicker = 0.7 + 0.3 * hash11(floor(bm_u.uTime * 14.0));
  let offset = (fbm2(vec2f(bm_in.vUv.y * 2.5 - bm_u.uTime * 2.5, floor(bm_u.uTime * 7.0)), 4.0) - 0.5) * 0.5;
  let d = abs(x - offset);
  let core = 0.006 / (d + 0.006);
  let glow = 0.04 / (d + 0.04);
  let bolt = pow(core, 1.5) * flicker;
  let halo = pow(glow, 2.0) * 0.5 * flicker;
  return vec4f(bolt * 0.8 + halo * 0.4, bolt * 0.9 + halo * 0.5, bolt + halo, 1.0);
}
`,
