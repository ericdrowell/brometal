...
const fireShader: CompiledShader<{ aPosition: 'vec3'; aUv: 'vec2' }, Record<string, never>, { uTime: 'float'; uAspect: 'float' }> = {
...
void main() {
  vec2 q = vec2(vUv.x * uAspect * 2.4, vUv.y * 2.0 - uTime * 1.3);
  float n = fbm2(q * 2.2, 5.0);
  float shape = clamp(1.25 - vUv.y * 1.6 + (n - 0.5) * 1.1, 0.0, 1.0);
  float heat = pow(shape, 1.6);
  fragColor = vec4(pow(heat, 0.8) * 1.05, pow(heat, 2.2), pow(heat, 6.0), 1.0);
}
`,
