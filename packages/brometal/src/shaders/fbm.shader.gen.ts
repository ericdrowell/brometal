...
const fbmShader: CompiledShader<{ aPosition: 'vec3'; aUv: 'vec2' }, Record<string, never>, { uTime: 'float'; uAspect: 'float' }> = {
...
void main() {
  ...
  vec2 drift = vec2(uTime * 0.25, uTime * 0.1);
  float n = fbm2(p + drift, 5.0);
  vec3 sky = vec3(0.16, 0.24, 0.45);
  vec3 cloud = vec3(0.95, 0.97, 1.0);
  fragColor = vec4(mix(sky, cloud, smoothstep(0.35, 0.75, n)), 1.0);
}
`,
...
fn fs_main(bm_in : BmVSOut) -> @location(0) vec4f {
  ...
  let drift = vec2f(bm_u.uTime * 0.25, bm_u.uTime * 0.1);
  let n = fbm2(p + drift, 5.0);
  let sky = vec3f(0.16, 0.24, 0.45);
  let cloud = vec3f(0.95, 0.97, 1.0);
  return vec4f(mix(sky, cloud, smoothstep(0.35, 0.75, n)), 1.0);
}
`,
