...
const fnEaseOutBackShader: CompiledShader<{ aPosition: 'vec3'; aUv: 'vec2' }, Record<string, never>, { uTime: 'float'; uAspect: 'float' }> = {
...
void main() {
  ...
  if (mx >= 0.0 && mx <= 1.0) {
    float yv = easeOutBack(clamp(mx, 0.0, 1.0));
    float d = abs(my - yv);
    color = mix(color, vec3(0.55, 0.6, 1.0), 1.0 - smoothstep(0.01, 0.03, d));
    float tt = fract(uTime * 0.45);
    vec2 dotVec = vec2((mx - tt) * 1.2, (my - easeOutBack(tt)) * 1.2);
...
export default fnEaseOutBackShader;
