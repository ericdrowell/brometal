...
import { smoothSubtract, sdCircle, sdBox2 } from 'brometal/shader-functions';

export const FnSmoothSubtract = shader({
  ...
  fragment({ uTime, uAspect }, { vUv }) {
    ...
    const box = sdBox2(p, vec2(0.22, 0.16));
    const d = smoothSubtract(circle, box, 0.06);
