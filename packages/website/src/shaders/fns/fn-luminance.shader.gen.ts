...
const fnLuminanceShader: CompiledShader<{ aPosition: 'vec3'; aUv: 'vec2' }, Record<string, never>, { uTime: 'float'; uAspect: 'float' }> = {
...
void main() {
  vec3 color = hsv2rgb(vec3(vUv.x, 0.85, 0.95));
  vec3 shown = color;
  if (vUv.x > 0.5) {
    float g = luminance(hsv2rgb(vec3(vUv.x - 0.5, 0.85, 0.95)));
    shown = vec3(g, g, g);
  }
...
fn fs_main(bm_in : BmVSOut) -> @location(0) vec4f {
  let color = hsv2rgb(vec3f(bm_in.vUv.x, 0.85, 0.95));
  var shown = color;
  if (bm_in.vUv.x > 0.5) {
    let g = luminance(hsv2rgb(vec3f(bm_in.vUv.x - 0.5, 0.85, 0.95)));
    shown = vec3f(g, g, g);
  }
...
export default fnLuminanceShader;
