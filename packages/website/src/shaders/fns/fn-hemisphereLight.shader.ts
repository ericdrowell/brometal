...
import { hemisphereLight } from 'brometal/shader-functions';

export const FnHemisphereLight = shader({
  ...
  fragment({ uTime, uAspect }, { vUv }) {
    ...
    if (r < 1) {
      ...
      color = hemisphereLight(normal, vec3(0.4, 0.6, 1), vec3(0.9, 0.5, 0.2));
    }
    return vec4(color, 1);
  },
});
