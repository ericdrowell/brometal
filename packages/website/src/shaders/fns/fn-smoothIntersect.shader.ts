...
import { smoothIntersect, sdCircle, sdBox2 } from 'brometal/shader-functions';

export const FnSmoothIntersect = shader({
  ...
  fragment({ uTime, uAspect }, { vUv }) {
    ...
    const circle = sdCircle(p.sub(vec2(cos(uTime) * 0.12, 0)), 0.2);
    const box = sdBox2(p, vec2(0.2, 0.14));
    const d = smoothIntersect(circle, box, 0.05);
