...
const fnHash21Shader: CompiledShader<{ aPosition: 'vec3'; aUv: 'vec2' }, Record<string, never>, { uTime: 'float'; uAspect: 'float' }> = {
...
void main() {
  ...
  float n = hash21(vec2(floor(q.x), floor(q.y) + floor(uTime) * 31.0));
...
fn fs_main(bm_in : BmVSOut) -> @location(0) vec4f {
  ...
  let n = hash21(vec2f(floor(q.x), floor(q.y) + floor(bm_u.uTime) * 31.0));
