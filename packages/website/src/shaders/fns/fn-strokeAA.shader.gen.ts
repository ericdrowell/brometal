...
const fnStrokeAAShader: CompiledShader<{ aPosition: 'vec3'; aUv: 'vec2' }, Record<string, never>, { uTime: 'float'; uAspect: 'float' }> = {
...
void main() {
  ...
  float s = strokeAA(sdHexagon(p, 0.26), 0.015, 0.005);
  fragColor = vec4(vec3(0.55, 0.9, 1.0) * s + vec3(0.08, 0.08, 0.12) * (1.0 - s), 1.0);
}
`,
...
fn fs_main(bm_in : BmVSOut) -> @location(0) vec4f {
  ...
  let s = strokeAA(sdHexagon(p, 0.26), 0.015, 0.005);
  return vec4f(vec3f(0.55, 0.9, 1.0) * s + vec3f(0.08, 0.08, 0.12) * (1.0 - s), 1.0);
}
`,
...
export default fnStrokeAAShader;
