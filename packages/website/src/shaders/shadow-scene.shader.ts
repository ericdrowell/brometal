...
export default shader({
  ...
  fragment(
    ...
    // The sample point is nudged along the normal first. A surface stores its
    // own distance in the map, so comparing against it is a tie, and which way
    // a tie falls is down to rounding — that is what shadow acne is. Pushing
    // the lookup a little off the surface breaks the tie in one direction
    // everywhere. Grazing light needs the most nudge, hence the ndl term.
    const slope = 1 + 2 * (1 - ndl);
    const lookupPoint = vWorld.add(n.scale(uNormalOffset * slope));
    const uv = targetUv(uLightViewProj.mul(vec4(lookupPoint, 1)));
    // World units, normalized after: subtracting the bias from the ratio makes
    // it uRange times larger than it reads, which quietly deletes the shadow
    // wherever an occluder sits close to what it is shadowing.
    const reference = (distance(lookupPoint, uLightPos) - uBias * slope) / uRange;

    // 3x3 PCF. One tap gives a stair-stepped edge at any map resolution; nine
    // spread over a texel or two average into a soft one, and widening that
    // spread is the softness control.
    const radius = uTexel * uSoftness;
    let visible = 0;
    for (let y = -1; y < 2; y += 1) {
      for (let x = -1; x < 2; x += 1) {
        const tap = uv.add(vec2(x, y).scale(radius));
        const nearest = texture(uShadowMap, tap).x;
        // The map is cleared to 1 — anything the light never drew reads as
        // maximally far, so points outside the light's frustum stay lit.
        visible += step(reference, nearest);
      }
    }
    const lit = 1 - (1 - visible / 9) * uShadowStrength;
